		  ...
		  crdtState.get_po_log(crdt_state)
			  .fold(crdt_state)(po_log => {
			  	val node_id = crdtState.get_node_id(crdt_state)
					...
					val (po_log_class1, msg_ops1) = make_msg(node_id, cluster_detail, user_msg, po_log_class0)
	...
	(PO_LOG_CLASS[NODE_ID, CLUSTER_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS],
	 MSG_OPS[NODE_ID, CLUSTER_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]) = {
	  val (tcsb_class, vclock) = tcsb.next(node_id, pologClass.get_tcsb_class(po_log_class))
		val msg_ops = msgOpr.asMSG_OPS(msgOpr.make_user_msg_ops(cluster_detail, nodeVCLOCK.make(node_id, vclock), user_msg))
	               ...
	               po_log_class: PO_LOG_CLASS[NODE_ID, CLUSTER_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS])
								(implicit vectorClock: VectorClock[NODE_ID],
								          ...
								          nodeVCLOCK: NodeVCLOCK[NODE_ID],
												  ...
												  updateCRDT: UpdateCRDT[NODE_ID, CLUSTER_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]):
	PO_LOG_CLASS[NODE_ID, CLUSTER_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS] = 
	  updateCRDT.update_causal_stable(updateCRDT.update_msg(msg_ops, po_log_class))
}
