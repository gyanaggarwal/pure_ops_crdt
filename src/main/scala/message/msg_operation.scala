...
trait MSGOperation[NODE_ID, CLUSTER_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS] {
  def empty_msg_list: MSG_LIST[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]

  def make_msg_vclock(node_vclock: NODE_VCLOCK[NODE_ID],
										  crdt_instance: CRDT_INSTANCE[CRDT_TYPE, CRDT_ID]):
	MESSAGE[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS] = 
	  MSG_VCLOCK(node_vclock, crdt_instance)

  def make_msg_ops(node_vclock: NODE_VCLOCK[NODE_ID],
									 crdt_instance: CRDT_INSTANCE[CRDT_TYPE, CRDT_ID],
								   crdt_ops: CRDT_OPS):
	MESSAGE[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS] = 
	  MSG_OPS(node_vclock, crdt_instance, crdt_ops)

  def make_user_msg_ops(node_vclock: NODE_VCLOCK[NODE_ID],
											  user_msg: USER_MSG[CRDT_TYPE, CRDT_ID, CRDT_OPS])
											 (implicit userMsg: UserMSG[CRDT_TYPE, CRDT_ID, CRDT_OPS]):
	MESSAGE[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS] = 
	  make_msg_ops(node_vclock, userMsg.get_crdt_instance(user_msg), userMsg.get_crdt_ops(user_msg))
		
	def get_crdt_ops(msg_ops: MSG_OPS[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]):
	CRDT_OPS = msg_ops.crdt_ops

  def asMSG_OPS(msg: MESSAGE[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]):
	MSG_OPS[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS] =
	  msg.asInstanceOf[MSG_OPS[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]]
			
  def asMSG_VCLOCK(msg: MESSAGE[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]):
	MSG_VCLOCK[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS] =
	  msg.asInstanceOf[MSG_VCLOCK[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]]

	def get_node_vclock(msg: MESSAGE[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]):
	...
	def get_crdt_instance(msg: MESSAGE[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS]):
	...
	def logical_clock(msg: MESSAGE[NODE_ID, CRDT_TYPE, CRDT_ID, CRDT_OPS])
